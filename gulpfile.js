...
gulp.task("move-components-fontawesome-6", (done) => {
  ...
  // Move font files
  gulp
    .src(`${config.packagesPath}/@fortawesome/fontawesome-pro/webfonts/**/*`)
...
gulp.task("move-components-athena-fonts", (done) => {
  gulp
    .src([`${config.packagesPath}/ucf-athena-framework/dist/fonts/**/*`])
    .pipe(gulp.dest(config.dist.fontPath));
  done();
});
...
gulp.task(
  "components",
  gulp.parallel("move-components-fontawesome-6", "move-components-athena-fonts")
);
...
gulp.task("js-build-highlights", () => {
  return buildJS(
    `${config.src.jsPath}/highlights-iconpicker.js`,
    config.dist.jsPath
  );
});
